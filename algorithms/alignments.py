...
class Alignment:
    ...
    def align_needleman_wunsch(self, match=1, mismatch=-1, gap=-1):
        ...
        decisions = {
            0: lambda i, j: (self.sequence_1[i - 1], self.sequence_2[j - 1], i - 1, j - 1),  # MATCH/MISMATCH
            1: lambda i, j: (self.sequence_1[i - 1], "_", i - 1, j),  # gap in lower sequence
            2: lambda i, j: ("_", self.sequence_2[j - 1], i, j - 1),  # gap in upper sequence
        }
...
if __name__ == '__main__':
    alignment_ = Alignment("GCATGCU", "GATTACA")
